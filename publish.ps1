...
$RepoName        = if ($env:REPO_NAME)        { $env:REPO_NAME }        else { "claude-code-export" }
$RepoVisibility  = if ($env:REPO_VISIBILITY)  { $env:REPO_VISIBILITY }  else { "public" }
$RepoDescription = if ($env:REPO_DESCRIPTION) { $env:REPO_DESCRIPTION } else { "Export Claude Code CLI sessions to HTML / Markdown / JSON / CSV, with the project's CLAUDE.md and any files the assistant touched." }
...
$Tracked = @("README.md","LICENSE",".gitignore",".gitattributes","pyproject.toml","claude_code_export.py","publish.sh","publish.ps1")
